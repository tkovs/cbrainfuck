...
char* interpreter(char* code_file_name, char* input_file_name) {
  ...
  // Initializes the "comment" variable using the constant _MAXLINELENGTH_
  comment = (char *) malloc(sizeof(char) * _MAXLINELENGTH_);
  ...
  while((c = fgetc(fcode)) != EOF) {
    ...
    switch (c) {
      case COMMENT:
        fgets(comment, 64, fcode);
        countscan += strlen(comment);
        break;
